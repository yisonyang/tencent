...
(function($) {
    $.fn.tabullet = function(options) {
        ...
        var tbody = $("<tbody/>").appendTo($(this)); //在table相应位置插入一个tbody标签
        // 第一行的添加
        ...
        $(metadata).each(function(i, v) {
            ...
            if (v.type === 'edit') {
                var td = $("<td/>")
                    .html('<button class="' + options.saveClass + '">add </button>')
                ...
                if (mv.type === 'delete') {
                    ...
                    td.find('button').click(function(event) {
                        if ($(this).attr('data-mode') === 'cancel') {
                            ...
                            var rowChildren = rowParent.find('input');
                            $(rowChildren).each(function(ri, rv) {
                                cancelData[$(rv).attr('name')] = v[$(rv).attr('name')];
                            });
                            cancelData[idMap] = $(rowParent).attr('data-tabullet-id');
                            options.action('edit', cancelData);
                            return;
                        } else {
                            alert("lalla");
                            tr.remove();
